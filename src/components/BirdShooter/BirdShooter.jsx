...
export default function BirdShootingGame({ onBack }) {
  ...
  // (၁) ဂိမ်းကစားနေစဉ် Header ထဲက Back Button အတွက် (Level ရွေးတဲ့နေရာကို ပြန်သွားရန်)
  ...
  // (၂) Bird Shooter Menu အောက်ခြေက Back Button အတွက် (App ရဲ့ Main Menu သို့ ထွက်ရန်)
  const exitToAppHome = () => {
    stopBg();
    onBack(); // Parent component က လက်ခံထားတဲ့ Exit function
  };
          ...
          {gameState === "playing" && (
            ...
            >
              <div className="flex items-center gap-4">
                <button
                  onClick={backToLevelSelect} // နာမည်အသစ်နဲ့ ချိတ်ပါ
        ...
        >
          ...
          {gameState === "playing" &&
            birds.map((bird) => (
              ...
              >
                <div
                  className={`relative z-20 px-4 py-1.5 rounded-xl font-bold shadow-lg border-2 text-lg transition-all duration-500 ${bird.status === "dying" ? "bg-yellow-400 text-white border-yellow-200 scale-0 opacity-0 rotate-12" : "bg-white text-slate-800 border-slate-100 shadow-md"}`}
                >
                  {bird.text}
                </div>
                <div
